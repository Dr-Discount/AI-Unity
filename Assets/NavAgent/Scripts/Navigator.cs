...
public class Navigator : MonoBehaviour
{
	...
	void Update()
	{
		...
		if (Mouse.current.rightButton.wasPressedThisFrame)
		{
			...
			{
				navAgent = hitInfo.collider.gameObject.GetComponent<NavAgent>() 
						   ?? hitInfo.collider.GetComponentInParent<NavAgent>();
				...
				if (navAgent != null)
				{
					if (marker != null)
					{
						...
						marker.transform.position = navAgent.transform.position + Vector3.up * 2f;
					}
				}
				else
				{
